...
exclude = ["samolot13", "samolot15", "samolot19"]
fnames = sorted(glob(PLANES_PATH+"/samolot??.jpg"))
planes_orig = [io.imread(fn) for fn in fnames if Path(fn).stem not in exclude ]
...
def draw_contour(image, ax=plt, lw=0.2):
    ...
    for c in cs2:
        ...
        ax.scatter(c[:,1], c[:,0], [lw]*len(c), c=col)
...
def collage(images, ncols=6, width=18, imparams={}, nplanes=None, contour=True, linewidth=0.2):
    sh = images[0].shape
    aspect = sh[0] / sh[1]
